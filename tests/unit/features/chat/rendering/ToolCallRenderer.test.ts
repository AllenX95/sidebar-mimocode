...
describe('ToolCallRenderer', () => {
  ...
  describe('renderToolCall', () => {
    ...
    it('should start expanded when requested', () => {
      ...
      const header = toolEl.querySelector('.sidebar-mimocode-tool-header');
      const content = toolEl.querySelector('.sidebar-mimocode-tool-content');

      expect(toolCall.isExpanded).toBe(true);
      expect(toolEl.hasClass('expanded')).toBe(true);
      expect(content?.hasClass('sidebar-mimocode-hidden')).toBe(false);
  ...
  describe('renderStoredToolCall', () => {
    ...
    it('renders AskUserQuestion answers from result text when resolvedAnswers is missing', () => {
      ...
      const answerEls = toolEl.querySelectorAll('.sidebar-mimocode-ask-review-a-text');
    ...
    it('renders AskUserQuestion options during fallback state', () => {
      ...
      const toolEl = renderStoredToolCall(parentEl, toolCall);
      const labelEls = toolEl.querySelectorAll('.sidebar-mimocode-ask-item-label');
      const descEls = toolEl.querySelectorAll('.sidebar-mimocode-ask-item-desc');
      const checkEls = toolEl.querySelectorAll('.sidebar-mimocode-ask-check');
  ...
  describe('updateToolCallResult', () => {
    it('should update status indicator', () => {
      ...
      const statusEl = toolEl.querySelector('.sidebar-mimocode-tool-status');
    ...
    it('shows raw AskUserQuestion result when answers cannot be parsed', () => {
      ...
      const promptEl = toolEl.querySelector('.sidebar-mimocode-ask-review-prompt');
  ...
  describe('WebSearch expanded rendering', () => {
    it('renders Codex search actions instead of the placeholder result text', () => {
      ...
      const toolEl = renderStoredToolCall(parentEl, toolCall);
      const lines = Array.from(toolEl.querySelectorAll('.sidebar-mimocode-tool-line')).map(line => line.textContent);
    ...
    it('renders Codex open_page actions from tool input even without a rich result body', () => {
      ...
      const toolEl = renderStoredToolCall(parentEl, toolCall);
      const links = toolEl.querySelectorAll('.sidebar-mimocode-tool-link');
      const lines = Array.from(toolEl.querySelectorAll('.sidebar-mimocode-tool-line')).map(line => line.textContent);

      expect(lines).toContain('Open page');
      expect(links).toHaveLength(1);
      expect(links[0].getAttribute('href')).toBe('https://example.com/docs');
      expect(links[0].querySelector('.sidebar-mimocode-tool-link-title')?.textContent).toBe('https://example.com/docs');
    });
  });
  ...
  describe('apply_patch expanded rendering', () => {
    it('renders parsed patch diffs from tool input', () => {
      ...
      const toolEl = renderStoredToolCall(parentEl, toolCall);
      const headers = Array.from(toolEl.querySelectorAll('.sidebar-mimocode-tool-patch-header')).map(el => el.textContent);
      const statusEl = toolEl.querySelector('.sidebar-mimocode-tool-status');
      const diffTexts = Array.from(toolEl.querySelectorAll('.sidebar-mimocode-diff-text')).map(el => el.textContent);

      expect(headers).toHaveLength(0);
      expect(statusEl?.hasClass('sidebar-mimocode-write-edit-stats')).toBe(true);
    ...
    it('renders stored apply_patch collapsed by default', () => {
      ...
      expect(content?.hasClass('sidebar-mimocode-hidden')).toBe(true);
    ...
    it('keeps the error status icon for failed apply_patch calls', () => {
      ...
      expect(statusEl?.hasClass('sidebar-mimocode-write-edit-stats')).toBe(false);
      expect(setIcon).toHaveBeenCalledWith(expect.anything(), 'x');
    });
    ...
    it('falls back to file changes when patch text is unavailable', () => {
      ...
      const toolEl = renderStoredToolCall(parentEl, toolCall);
      const lines = Array.from(toolEl.querySelectorAll('.sidebar-mimocode-tool-line')).map(el => el.textContent);
  ...
  describe('renderTodoWriteResult', () => {
    it('should render todo items', () => {
      ...
      expect(container.hasClass('sidebar-mimocode-todo-panel-content')).toBe(true);
      expect(container.hasClass('sidebar-mimocode-todo-list-container')).toBe(true);
    });
  ...
  describe('updateToolCallResult for TodoWrite', () => {
    it('should update todo status and content', () => {
      ...
      const statusEl = parentEl.querySelector('.sidebar-mimocode-tool-status');
      expect(statusEl?.hasClass('status-completed')).toBe(true);
    });
