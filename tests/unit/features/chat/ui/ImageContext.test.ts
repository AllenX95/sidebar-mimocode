...
function createContainerWithInputWrapper(): { container: any; inputWrapper: any } {
  ...
  const inputWrapper = container.createDiv({ cls: 'sidebar-mimocode-input-wrapper' });
...
describe('ImageContextManager', () => {
  ...
  describe('constructor with previewContainerEl', () => {
    it('should use previewContainerEl when provided', () => {
      ...
      const previewEl = previewContainer.querySelector('.sidebar-mimocode-image-preview');
      expect(previewEl).not.toBeNull();
    });

    it('should insert image preview before file indicator if present', () => {
      const previewContainer = createMockEl();
      const fileIndicator = previewContainer.createDiv({ cls: 'sidebar-mimocode-file-indicator' });
      ...
      const previewIdx = children.findIndex((el: any) => el.hasClass?.('sidebar-mimocode-image-preview'));
...
describe('ImageContextManager - Private Helpers', () => {
  ...
  describe('Image preview rendering', () => {
    ...
    it('renderImagePreview should create chip with thumbnail, info, and remove button', () => {
      ...
      expect(chipEl.hasClass('sidebar-mimocode-image-chip')).toBe(true);

      const thumbEl = chipEl.querySelector('.sidebar-mimocode-image-thumb');
      expect(thumbEl).not.toBeNull();

      const infoEl = chipEl.querySelector('.sidebar-mimocode-image-info');
      expect(infoEl).not.toBeNull();

      const removeEl = chipEl.querySelector('.sidebar-mimocode-image-remove');
    ...
    it('remove button should delete the image and update preview', () => {
      ...
      const removeEl = firstChip.querySelector('.sidebar-mimocode-image-remove');
