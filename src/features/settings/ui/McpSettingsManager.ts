...
export class McpSettingsManager {
  ...
  private render() {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'sidebar-mimocode-mcp-header' });
    headerEl.createSpan({ text: 'MCP Servers', cls: 'sidebar-mimocode-mcp-label' });

    const addContainer = headerEl.createDiv({ cls: 'sidebar-mimocode-mcp-add-container' });
    ...
    const dropdown = addContainer.createDiv({ cls: 'sidebar-mimocode-mcp-add-dropdown' });

    const stdioOption = dropdown.createDiv({ cls: 'sidebar-mimocode-mcp-add-option' });
    setIcon(stdioOption.createSpan({ cls: 'sidebar-mimocode-mcp-add-option-icon' }), 'terminal');
    ...
    const httpOption = dropdown.createDiv({ cls: 'sidebar-mimocode-mcp-add-option' });
    setIcon(httpOption.createSpan({ cls: 'sidebar-mimocode-mcp-add-option-icon' }), 'globe');
    ...
    const importOption = dropdown.createDiv({ cls: 'sidebar-mimocode-mcp-add-option' });
    setIcon(importOption.createSpan({ cls: 'sidebar-mimocode-mcp-add-option-icon' }), 'clipboard-paste');
    ...
    if (this.servers.length === 0) {
      const emptyEl = this.containerEl.createDiv({ cls: 'sidebar-mimocode-mcp-empty' });
      emptyEl.setText('No mcp servers configured. Click "add" to add one.');
      return;
    }

    const listEl = this.containerEl.createDiv({ cls: 'sidebar-mimocode-mcp-list' });
  ...
  private renderServerItem(listEl: HTMLElement, server: ManagedMcpServer) {
    const itemEl = listEl.createDiv({ cls: 'sidebar-mimocode-mcp-item' });
    if (!server.enabled) {
      itemEl.addClass('sidebar-mimocode-mcp-item-disabled');
    }

    const statusEl = itemEl.createDiv({ cls: 'sidebar-mimocode-mcp-status' });
    statusEl.addClass(
      server.enabled ? 'sidebar-mimocode-mcp-status-enabled' : 'sidebar-mimocode-mcp-status-disabled'
    );

    const infoEl = itemEl.createDiv({ cls: 'sidebar-mimocode-mcp-info' });

    const nameRow = infoEl.createDiv({ cls: 'sidebar-mimocode-mcp-name-row' });

    const nameEl = nameRow.createSpan({ cls: 'sidebar-mimocode-mcp-name' });
    nameEl.setText(server.name);

    const serverType = getMcpServerType(server.config);
    const typeEl = nameRow.createSpan({ cls: 'sidebar-mimocode-mcp-type-badge' });
    typeEl.setText(serverType);

    if (server.contextSaving) {
      const csEl = nameRow.createSpan({ cls: 'sidebar-mimocode-mcp-context-saving-badge' });
      csEl.setText('@');
      csEl.setAttribute('title', 'Context-saving: mention with @' + server.name + ' to enable');
    }

    const previewEl = infoEl.createDiv({ cls: 'sidebar-mimocode-mcp-preview' });
    ...
    const actionsEl = itemEl.createDiv({ cls: 'sidebar-mimocode-mcp-actions' });

    const testBtn = actionsEl.createEl('button', {
      cls: 'sidebar-mimocode-mcp-action-btn',
    ...
    const deleteBtn = actionsEl.createEl('button', {
      cls: 'sidebar-mimocode-mcp-action-btn sidebar-mimocode-mcp-delete-btn',
