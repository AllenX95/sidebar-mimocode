...
export function refreshTabProviderUI(tab: TabData, plugin: SidebarMimocodePlugin): void {
 ...
 */
export function applyProviderUIGating(tab: TabData, plugin: SidebarMimocodePlugin): void {
  const capabilities = getTabCapabilities(tab, plugin);
...
function syncTabProviderServices(
  ...
  const taskResultInterpreter = ProviderRegistry.getTaskResultInterpreter(tab.providerId);
  tab.controllers.streamController?.setSubagentTaskResultInterpreter(taskResultInterpreter);
}
  ...
  tab.controllers.conversationController = new ConversationController(
    {
      ...
      getStreamController: () => tab.controllers.streamController,
...
export async function destroyTab(tab: TabData): Promise<void> {
  ...
  void tab.controllers.streamController?.orphanAllActiveSubagents();
  if (tab.controllers.streamController) {
    tab.controllers.streamController.clearSubagents();
  }
...
export function setupServiceCallbacks(tab: TabData, plugin: SidebarMimocodePlugin): void {
  if (tab.service && tab.controllers.inputController) {
    ...
    tab.service.setSubagentHookProvider(
      () => ({
        hasRunning: tab.controllers.streamController?.hasRunningSubagents() ?? false,
      })
    );
...
async function renderAutoTriggeredTurn(tab: TabData, result: AutoTurnResult): Promise<void> {
  ...
  try {
    ...
    if (hasVisibleContent) {
      ...
      tab.controllers.streamController?.resetSubagentStreamingState();
