...
export class NavigationSidebar {
  ...
  constructor(
    ...
    this.container = this.parentEl.createDiv({ cls: 'sidebar-mimocode-nav-sidebar' });

    // Create buttons
    this.topBtn = this.createButton('sidebar-mimocode-nav-btn-top', 'chevrons-up', 'Scroll to top');
    this.prevBtn = this.createButton('sidebar-mimocode-nav-btn-prev', 'chevron-up', 'Previous message');
    this.nextBtn = this.createButton('sidebar-mimocode-nav-btn-next', 'chevron-down', 'Next message');
    this.bottomBtn = this.createButton('sidebar-mimocode-nav-btn-bottom', 'chevrons-down', 'Scroll to bottom');
  ...
  private scrollToMessage(direction: 'prev' | 'next'): void {
    const messages = Array.from(this.messagesEl.querySelectorAll<HTMLElement>('.sidebar-mimocode-message-user'));
