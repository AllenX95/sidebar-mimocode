...
export class StatusPanel {
  ...
  private createPanel(): void {
    ...
    this.panelEl.className = 'sidebar-mimocode-status-panel';

    // Bash output container - hidden by default
    this.bashOutputContainerEl = ownerDocument.createElement('div');
    this.bashOutputContainerEl.className = 'sidebar-mimocode-status-panel-bash sidebar-mimocode-hidden';

    this.bashHeaderEl = ownerDocument.createElement('div');
    this.bashHeaderEl.className = 'sidebar-mimocode-tool-header sidebar-mimocode-status-panel-bash-header';
    ...
    this.bashContentEl.className = 'sidebar-mimocode-status-panel-bash-content';
    ...
    this.todoContainerEl.className = 'sidebar-mimocode-status-panel-todos sidebar-mimocode-hidden';
    this.panelEl.appendChild(this.todoContainerEl);

    // Todo header (collapsed view)
    this.todoHeaderEl = ownerDocument.createElement('div');
    this.todoHeaderEl.className = 'sidebar-mimocode-status-panel-header';
    ...
    this.todoContentEl.className = 'sidebar-mimocode-status-panel-content sidebar-mimocode-todo-list-container sidebar-mimocode-hidden';
  ...
  updateTodos(todos: TodoItem[] | null): void {
    ...
    if (!todos || todos.length === 0) {
      this.todoContainerEl.addClass('sidebar-mimocode-hidden');
      this.todoHeaderEl.empty();
      this.todoContentEl.empty();
      return;
    }

    this.todoContainerEl.removeClass('sidebar-mimocode-hidden');
  ...
  private renderTodoHeader(completedCount: number, totalCount: number, currentTask: TodoItem | undefined): void {
    ...
    icon.className = 'sidebar-mimocode-status-panel-icon';
    setIcon(icon, getToolIcon(TOOL_TODO_WRITE));
    this.todoHeaderEl.appendChild(icon);

    // Label
    const label = ownerDocument.createElement('span');
    label.className = 'sidebar-mimocode-status-panel-label';
    ...
    if (!this.isTodoExpanded) {
      ...
      if (completedCount === totalCount && totalCount > 0) {
        ...
        status.className = 'sidebar-mimocode-status-panel-status status-completed';
      ...
      if (currentTask) {
        ...
        current.className = 'sidebar-mimocode-status-panel-current';
  ...
  private updateTodoDisplay(): void {
    ...
    // Show/hide content
    this.todoContentEl.toggleClass('sidebar-mimocode-hidden', !this.isTodoExpanded);
  ...
  private renderBashOutputs(options: { scroll?: boolean } = {}): void {
    ...
    if (this.currentBashOutputs.size === 0) {
      this.bashOutputContainerEl.addClass('sidebar-mimocode-hidden');
      return;
    }

    this.bashOutputContainerEl.removeClass('sidebar-mimocode-hidden');
    ...
    headerIconEl.className = 'sidebar-mimocode-tool-icon';
    ...
    headerLabelEl.className = 'sidebar-mimocode-tool-label';
    ...
    previewEl.className = 'sidebar-mimocode-tool-current';
    previewEl.classList.toggle('sidebar-mimocode-hidden', !this.isBashExpanded);
    this.bashHeaderEl.appendChild(previewEl);

    const summaryStatusEl = ownerDocument.createElement('span');
    summaryStatusEl.className = 'sidebar-mimocode-tool-status';
    if (!this.isBashExpanded && latest) {
      ...
      summaryStatusEl.classList.add('sidebar-mimocode-hidden');
    }
    ...
    actionsEl.className = 'sidebar-mimocode-status-panel-bash-actions';
    ...
    this.bashContentEl.toggleClass('sidebar-mimocode-hidden', !this.isBashExpanded);
  ...
  private renderBashEntry(info: PanelBashOutput, ownerDocument: Document): HTMLElement {
    ...
    entryEl.className = 'sidebar-mimocode-tool-call sidebar-mimocode-status-panel-bash-entry';

    const entryHeaderEl = ownerDocument.createElement('div');
    entryHeaderEl.className = 'sidebar-mimocode-tool-header';
    ...
    entryIconEl.className = 'sidebar-mimocode-tool-icon';
    ...
    entryLabelEl.className = 'sidebar-mimocode-tool-label';
    ...
    entryStatusEl.className = 'sidebar-mimocode-tool-status';
    ...
    contentEl.className = 'sidebar-mimocode-tool-content';
    const isEntryExpanded = this.bashEntryExpanded.get(info.id) ?? true;
    contentEl.classList.toggle('sidebar-mimocode-hidden', !isEntryExpanded);
    ...
    rowEl.className = 'sidebar-mimocode-tool-result-row';

    const textEl = ownerDocument.createElement('span');
    textEl.className = 'sidebar-mimocode-tool-result-text';
  ...
  private appendActionButton(
    ...
    el.className = `sidebar-mimocode-status-panel-bash-action sidebar-mimocode-status-panel-bash-action-${name}`;
