...
export function autoResizeTextarea(textarea: HTMLTextAreaElement): void {
  const viewHeight = textarea.closest('.sidebar-mimocode-container')?.clientHeight ?? window.innerHeight;
  const maxHeight = calculateTextareaMaxHeight(viewHeight);

  textarea.setCssProps({
    '--sidebar-mimocode-textarea-min-height': `${TEXTAREA_BASE_MIN_HEIGHT}px`,
    '--sidebar-mimocode-textarea-max-height': `${maxHeight}px`,
  });
  ...
  textarea.setCssProps({
    '--sidebar-mimocode-textarea-min-height': `${minHeight}px`,
    '--sidebar-mimocode-textarea-max-height': `${maxHeight}px`,
  });
}
