...
export class MessageRenderer {
  ...
  addMessage(msg: ChatMessage): HTMLElement {
    ...
    const msgEl = this.messagesEl.createDiv({
      cls: `sidebar-mimocode-message sidebar-mimocode-message-${msg.role}`,
      attr: {
    ...
    const contentEl = msgEl.createDiv({ cls: 'sidebar-mimocode-message-content', attr: { dir: 'auto' } });

    if (msg.role === 'user') {
      const textToShow = this.getUserMessageTextToShow(msg);
      if (textToShow) {
        const textEl = contentEl.createDiv({ cls: 'sidebar-mimocode-text-block' });
  ...
  updateLiveUserMessage(msg: ChatMessage): void {
    ...
    const toolbar = msgEl.querySelector<HTMLElement>('.sidebar-mimocode-user-msg-actions');
    if (toolbar) {
      toolbar.querySelectorAll('.sidebar-mimocode-user-msg-copy-btn').forEach((el) => el.remove());
    }
  ...
  private renderInterruptMessage(): void {
    const msgEl = this.messagesEl.createDiv({ cls: 'sidebar-mimocode-message sidebar-mimocode-message-assistant' });
    const contentEl = msgEl.createDiv({ cls: 'sidebar-mimocode-message-content', attr: { dir: 'auto' } });
    this.appendInterruptIndicator(contentEl);
  }
  ...
  private appendInterruptIndicator(contentEl: HTMLElement): void {
    const textEl = contentEl.createDiv({ cls: 'sidebar-mimocode-text-block' });
    textEl.createSpan({ cls: 'sidebar-mimocode-interrupted', text: 'Interrupted' });
    textEl.appendText(' ');
    textEl.createSpan({
      cls: 'sidebar-mimocode-interrupted-hint',
      text: '\u00B7 What should MiMo-Code do instead?',
    });
  }
  ...
  private renderAssistantContent(msg: ChatMessage, contentEl: HTMLElement): void {
    if (msg.contentBlocks && msg.contentBlocks.length > 0) {
      ...
      for (const block of msg.contentBlocks) {
        if (block.type === 'thinking') {
          ...
          const boundaryEl = contentEl.createDiv({ cls: 'sidebar-mimocode-compact-boundary' });
          boundaryEl.createSpan({ cls: 'sidebar-mimocode-compact-boundary-label', text: 'Conversation compacted' });
    ...
    if (msg.durationSeconds && msg.durationSeconds > 0 && !hasCompactBoundary) {
      ...
      const footerEl = contentEl.createDiv({ cls: 'sidebar-mimocode-response-footer' });
      footerEl.createSpan({
  ...
  renderMessageImages(containerEl: HTMLElement, images: ImageAttachment[]): void {
    const imagesEl = containerEl.createDiv({ cls: 'sidebar-mimocode-message-images' });

    for (const image of images) {
      const imageWrapper = imagesEl.createDiv({ cls: 'sidebar-mimocode-message-image' });
  ...
  showFullImage(image: ImageAttachment): void {
    ...
    const overlay = ownerDocument.body.createDiv({ cls: 'sidebar-mimocode-image-modal-overlay' });
    const modal = overlay.createDiv({ cls: 'sidebar-mimocode-image-modal' });
    ...
    const closeBtn = modal.createDiv({ cls: 'sidebar-mimocode-image-modal-close' });
    closeBtn.setText('\u00D7');
  ...
  async renderContent(
    ...
    try {
      ...
      el.querySelectorAll('pre').forEach((pre) => {
        ...
        if (pre.parentElement?.classList.contains('sidebar-mimocode-code-wrapper')) return;

        // Create wrapper
        const wrapper = createEl('div', { cls: 'sidebar-mimocode-code-wrapper' });
        ...
        if (code) {
          ...
          if (match) {
            ...
            const label = createEl('span', {
              cls: 'sidebar-mimocode-code-lang-label',
              text: match[1],
            });
      ...
      el.createDiv({
        cls: 'sidebar-mimocode-render-error',
  ...
  addTextCopyButton(textEl: HTMLElement, markdown: string): void {
    const copyBtn = textEl.createSpan({ cls: 'sidebar-mimocode-text-copy-btn' });
  ...
  refreshActionButtons(msg: ChatMessage, allMessages?: ChatMessage[], index?: number): void {
    ...
    if (this.rewindCallback && !msgEl.querySelector('.sidebar-mimocode-message-rewind-btn')) {
      this.addRewindButton(msgEl, msg.id);
    }
    if (this.forkCallback && !msgEl.querySelector('.sidebar-mimocode-message-fork-btn')) {
      this.addForkButton(msgEl, msg.id);
    }
  ...
  private cleanupLiveMessageEl(msgId: string, msgEl: HTMLElement): void {
    const needsRewind = this.rewindCallback && !msgEl.querySelector('.sidebar-mimocode-message-rewind-btn');
    const needsFork = this.forkCallback && !msgEl.querySelector('.sidebar-mimocode-message-fork-btn');
  ...
  private getOrCreateActionsToolbar(msgEl: HTMLElement): HTMLElement {
    const existing = msgEl.querySelector<HTMLElement>('.sidebar-mimocode-user-msg-actions');
    if (existing) return existing;
    return msgEl.createDiv({ cls: 'sidebar-mimocode-user-msg-actions' });
  }

  private addUserCopyButton(msgEl: HTMLElement, content: string): void {
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const copyBtn = toolbar.createSpan({ cls: 'sidebar-mimocode-user-msg-copy-btn' });
    setIcon(copyBtn, 'copy');
  ...
  private addRewindButton(msgEl: HTMLElement, messageId: string): void {
    ...
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const btn = toolbar.createSpan({ cls: 'sidebar-mimocode-message-rewind-btn' });
  ...
  private addForkButton(msgEl: HTMLElement, messageId: string): void {
    ...
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const btn = toolbar.createSpan({ cls: 'sidebar-mimocode-message-fork-btn' });
    if (toolbar.firstChild !== btn) toolbar.insertBefore(btn, toolbar.firstChild);
