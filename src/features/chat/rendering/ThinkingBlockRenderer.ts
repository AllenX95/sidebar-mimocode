...
export function createThinkingBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'sidebar-mimocode-thinking-block' });

  // Header (clickable to expand/collapse)
  const header = wrapperEl.createDiv({ cls: 'sidebar-mimocode-thinking-header' });
  ...
  // Label with timer
  const labelEl = header.createSpan({ cls: 'sidebar-mimocode-thinking-label' });
  ...
  const contentEl = wrapperEl.createDiv({ cls: 'sidebar-mimocode-thinking-content' });
...
export function finalizeThinkingBlock(state: ThinkingBlockState): number {
  ...
  const header = state.wrapperEl.querySelector('.sidebar-mimocode-thinking-header');
  if (header) {
