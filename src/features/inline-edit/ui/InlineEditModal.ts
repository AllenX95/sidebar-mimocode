...
export function buildInlineEditInputDecorations(options: {
  ...
  return Decoration.set([
    Decoration.line({
      class: 'sidebar-mimocode-inline-input-line',
    }).range(lineStart),
    Decoration.widget({
...
function getDiffBlockClass(type: DiffOp['type']): string {
  switch (type) {
    case 'delete':
      return 'sidebar-mimocode-diff-del';
    case 'insert':
      return 'sidebar-mimocode-diff-ins';
    default:
      return 'sidebar-mimocode-diff-equal';
  }
}
...
class InlineEditController {
  ...
  createInputDOM(): HTMLElement {
    ...
    container.className = 'sidebar-mimocode-inline-input-container';
    this.containerEl = container;

    this.agentReplyEl = ownerDocument.createElement('div');
    this.agentReplyEl.className = 'sidebar-mimocode-inline-agent-reply sidebar-mimocode-hidden';
    container.appendChild(this.agentReplyEl);

    const inputWrap = ownerDocument.createElement('div');
    inputWrap.className = 'sidebar-mimocode-inline-input-wrap';
    container.appendChild(inputWrap);

    this.inputEl = ownerDocument.createElement('input');
    this.inputEl.type = 'text';
    this.inputEl.className = 'sidebar-mimocode-inline-input';
    ...
    this.spinnerEl.className = 'sidebar-mimocode-inline-spinner sidebar-mimocode-hidden';
  ...
  createDiffPreviewDOM(diffOps: DiffOp[]): HTMLElement {
    ...
    previewEl.className = 'sidebar-mimocode-inline-diff-preview';

    const bodyEl = ownerDocument.createElement('div');
    bodyEl.className = 'sidebar-mimocode-inline-diff-preview-body markdown-rendered';
    previewEl.appendChild(bodyEl);

    const actionsEl = ownerDocument.createElement('div');
    actionsEl.className = 'sidebar-mimocode-inline-preview-actions';
  ...
  private createPreviewActionButton(
    ...
    button.type = 'button';
    button.className = `sidebar-mimocode-inline-preview-action ${variant}`;
  ...
  private async renderMarkdownDiffPreview(container: HTMLElement, diffOps: DiffOp[]): Promise<void> {
    ...
    for (const document of buildMarkdownDiffDocuments(diffOps)) {
      ...
      opEl.className = `sidebar-mimocode-diff-block ${getDiffBlockClass(document.type)}`;
  ...
  private async generate() {
    ...
    this.spinnerEl.removeClass('sidebar-mimocode-hidden');
    ...
    this.spinnerEl.addClass('sidebar-mimocode-hidden');
  ...
  private showAgentReply(message: string) {
    ...
    replyEl.removeClass('sidebar-mimocode-hidden');
    replyEl.empty();
