...
export class MentionDropdownController {
  ...
  constructor(
    ...
    this.dropdown = new SelectableDropdown<MentionItem>(this.containerEl, {
      listClassName: 'sidebar-mimocode-mention-dropdown',
      itemClassName: 'sidebar-mimocode-mention-item',
      emptyClassName: 'sidebar-mimocode-mention-empty',
      fixed: this.fixed,
      fixedClassName: 'sidebar-mimocode-mention-dropdown-fixed',
    });
  }
  ...
  private renderMentionDropdown(): void {
    this.dropdown.render({
      ...
      renderItem: (item, itemEl) => {
        const iconEl = itemEl.createSpan({ cls: 'sidebar-mimocode-mention-icon' });
        switch (item.type) {
          case 'mcp-server':
        ...
        const textEl = itemEl.createSpan({ cls: 'sidebar-mimocode-mention-text' });

        switch (item.type) {
          case 'mcp-server':
            textEl.createSpan({ cls: 'sidebar-mimocode-mention-name' }).setText(`@${item.name}`);
            break;
          case 'agent-folder':
            textEl.createSpan({
              cls: 'sidebar-mimocode-mention-name sidebar-mimocode-mention-name-agent-folder',
            }).setText(`@${item.name}/`);
            break;
          case 'agent': {
            // Show ID (which is namespaced for plugin agents) for consistency with inserted text
            textEl.createSpan({
              cls: 'sidebar-mimocode-mention-name sidebar-mimocode-mention-name-agent',
            }).setText(`@${item.id}`);
            if (item.description) {
              textEl.createSpan({ cls: 'sidebar-mimocode-mention-agent-desc' }).setText(item.description);
            }
            break;
          }
          case 'context-folder':
            textEl.createSpan({
              cls: 'sidebar-mimocode-mention-name sidebar-mimocode-mention-name-folder',
            }).setText(`@${item.name}/`);
            break;
          case 'context-file':
            textEl.createSpan({
              cls: 'sidebar-mimocode-mention-name sidebar-mimocode-mention-name-context',
            }).setText(item.name);
            break;
          case 'folder':
            textEl.createSpan({
            ...
            break;
          default:
            textEl.createSpan({ cls: 'sidebar-mimocode-mention-path' }).setText(item.path || item.name);
        }
      },
  ...
  private positionFixed(): void {
    ...
    dropdownEl.setCssProps({
      '--sidebar-mimocode-fixed-dropdown-bottom': `${window.innerHeight - inputRect.top + 4}px`,
      '--sidebar-mimocode-fixed-dropdown-left': `${inputRect.left}px`,
      '--sidebar-mimocode-fixed-dropdown-width': `${Math.max(inputRect.width, 280)}px`,
    });
  }
