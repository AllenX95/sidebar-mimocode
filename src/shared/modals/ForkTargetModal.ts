...
class ForkTargetModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('sidebar-mimocode-fork-target-modal');

    const list = this.contentEl.createDiv({ cls: 'sidebar-mimocode-fork-target-list' });
  ...
  private createOption(container: HTMLElement, target: ForkTarget, label: string): void {
    const item = container.createDiv({ cls: 'sidebar-mimocode-fork-target-option', text: label });
