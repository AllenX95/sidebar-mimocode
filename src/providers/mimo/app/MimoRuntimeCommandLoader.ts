...
export class MimoRuntimeCommandLoader implements ProviderRuntimeCommandLoader {
  isAvailable(settings: Record<string, unknown>): boolean {
    return getMimoProviderSettings(settings).enabled;
  }
  ...
  async loadCommands(context: ProviderRuntimeCommandLoaderContext) {
    ...
    const canReuseRuntime = context.runtime?.providerId === 'mimo'
      && !shouldWarmPreSessionConversation;
    const runtime = canReuseRuntime
      ? context.runtime!
      : new MimoChatRuntime(context.plugin);
    ...
    try {
      if (context.conversation) {
        ...
        // without binding a persisted MiMo-Code session to the tab.
