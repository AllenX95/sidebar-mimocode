import type { MimoDiscoveredModel, MimoThinkingOptionsByModel } from '../models';
import type { MimoMode } from '../modes';
...
export function sameModes(left: MimoMode[], right: MimoMode[]): boolean {
  if (left.length !== right.length) {
    return false;
  }
...
export function sameDiscoveredModels(
  left: MimoDiscoveredModel[],
  right: MimoDiscoveredModel[],
): boolean {
  if (left.length !== right.length) {
    return false;
  }
...
export function sameThinkingOptionsByModel(
  left: MimoThinkingOptionsByModel,
  right: MimoThinkingOptionsByModel,
): boolean {
  const leftEntries = Object.entries(left);
  if (leftEntries.length !== Object.keys(right).length) {
    return false;
