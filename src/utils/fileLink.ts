/**
 * SidebarMimocode - File Link Utilities
 *
...
function createWikilink(
  ...
  link.className = 'sidebar-mimocode-file-link internal-link';
...
function repairEmptyInternalLink(app: App, link: HTMLAnchorElement): void {
  ...
  link.classList.add('sidebar-mimocode-file-link');
 ...
 * Handles both our custom .sidebar-mimocode-file-link and Obsidian's .internal-link.
 */
export function registerFileLinkHandler(
  ...
  component.registerDomEvent(container, 'click', (event: MouseEvent) => {
    ...
    const link = target.closest('.sidebar-mimocode-file-link, .internal-link') as HTMLAnchorElement;
  ...
  const walker = container.ownerDocument.createTreeWalker(
    ...
    NodeFilter.SHOW_TEXT,
    {
      acceptNode(node) {
        ...
        if (parent.closest('pre, code, a, .sidebar-mimocode-file-link, .internal-link')) {
          return NodeFilter.FILTER_REJECT;
        }
